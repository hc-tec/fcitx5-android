...
internal object FunctionKitPermissionPolicy {
    fun isEnabled(permission: String, prefs: AppPrefs.FunctionKit): Boolean =
        when (permission) {
            ...
            "network.fetch" -> prefs.remoteInferenceEnabled.getValue()
            "ai.chat" -> prefs.remoteInferenceEnabled.getValue()
            "ai.agent.list" -> prefs.remoteInferenceEnabled.getValue()
            "ai.agent.run" -> prefs.remoteInferenceEnabled.getValue()
